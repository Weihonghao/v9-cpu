...
setup_user_paging()
{
  ...
  int i;
  int *pg = pg_dir + 1024 * 17;
  pg_dir[USRSTART >> 22] = (int)pg | PTE_P | PTE_W | PTE_U;
  for (i=0;i<1024;i++)
  {
    pg[i] = (USRPHY + (i << 12)) | PTE_P | PTE_W | PTE_U;
  }
}
...
setup_kernel_paging()
{
  ...
  int i;
  // set the start addr of the 1-level page table
  pg_dir = (int *)((((int)&pg_mem) + 4095) & -4096);
  // set the 1-level page table
  for (i=0;i<1024;i++) 
  {
    pg_dir[i] = 0;
  }
  ...
  pg_dir[0] = (int)(pg_tbl[0]) | PTE_P | PTE_W;
  for (i=0;i<16;i++) 
  {
    pg_dir[(KERSTART >> 22) + i] = (int)(pg_tbl[i]) | PTE_P | PTE_W;
  }
  // set the 2-level page table
  for (i=0;i<16*1024;i++) 
  {
